...
public class SiriusExportTask extends AbstractTask {
    ...
    private void writeHeader(BufferedWriter writer, PeakListRow row, RawDataFile raw, char polarity, MsType msType, int scanNumber) throws IOException {
        ...
        writer.write(String.valueOf(medianMzOf(row.getBestPeak())));
        writer.newLine();
        writer.write("CHARGE=");
    ...
    private int getIndexOfFeaturePeak(Feature bestPeak) {
        final CoelutingPeaks peaks = bestPeak.get(CoelutingPeaks.class, null);
        if (peaks == null) return -1;
        else {
            double minDev = Double.POSITIVE_INFINITY;
            int p = 0;
            for (int i = 0; i < peaks.size(); ++i) {
                final double dev = Math.abs(peaks.getMz(i) - bestPeak.getMZ());
                if (dev < minDev) {
                    minDev = dev;
                    p = i;
                }
    ...
    private double medianMzOf(Feature bestPeak) {
        final CoelutingPeaks peaks = bestPeak.get(CoelutingPeaks.class, null);
        if (peaks == null) return bestPeak.getMZ();
        else return peaks.getMz(getIndexOfFeaturePeak(bestPeak));
    }

    private void writeSearchAdducts(BufferedWriter writer, PeakListRow row) throws IOException {
        final Feature[] features = row.getPeaks();
        Arrays.sort(features, new PeakSorter(SortingProperty.Height, SortingDirection.Descending));
        for (Feature feature : features) {
            final CoelutingPeaks peaks = feature.get(CoelutingPeaks.class, null);
            if (peaks != null) {
                CoelutingPeaks.Annotation ano = peaks.getAnnotation(getIndexOfFeaturePeak(feature));
                if (ano == null || ano.isUnknown()) continue;
                writer.write("ADDUCT=");
                writer.write(ano.getAdductType());
                writer.newLine();
                return;
            }
        }
